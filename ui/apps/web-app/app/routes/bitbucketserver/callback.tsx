...
export const loader: LoaderFunction = async ({ request }: LoaderArgs) => {
  ...
  const { data } = await setOAuthDataSource(`${code}`, 'Bitbucket');
  ...
  if (redirectUrl) {
    ...
    dataSources = [...dataSources, 'Bitbucket'];
    session.set('dataSources', dataSources);
    headers.append('Set-Cookie', await commitSession(session));
    return redirect('/onboarding/data-sources/step/2/source/Bitbucket', { headers });
  } else {
